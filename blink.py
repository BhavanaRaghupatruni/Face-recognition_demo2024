...
# List to hold images and class names
images = []
classNames = []
...
while True:
    ...
    # If blink count is 0, prompt for valid image
    if blink_counter == 0:
    ...
    # If 2 or more blinks are detected, proceed to face recognition
    if blink_counter >= CONSEC_FRAMES:
        for encode_face, faceloc in zip(encoded_faces, faces_in_frame):
            ...
            if name != "Unknown" and is_live:
                ...
                # Break after marking attendance for the first recognized face
                break

        # Exit the loop after marking attendance and showing the result
        cv2.imshow('Webcam', img)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
